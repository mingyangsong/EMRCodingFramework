...
public class hieraticalRule implements Serializable {
	...
	void readRulesThroughFile(String path) throws IOException {
		...
		while (ruleParser.readOneLine(oneRow)) {
			...
			if (currentGroup != oneRow.group
					|| !currentConcepId.equals(oneRow.conceptId)) {
			...
			newRule.setTimeRelatedSnowMed(oneRow.conceptId);// FIX ME current
															// only think all
															// center snow is
															// time related
			...
			if (oneRow.onset != null) {
				if (!pool.addtimeRleatedMappingRule(newRule.timeRelatedSnowMed,
						newRule))
					stdOut.setString("error r2");
			} else {
		...
		pool.print();
		stdOut.setString("input: " + numOfRules + " rules");
		stdOut.setString("input: " + pool.size() + " snowMeds");
		stdOut.setString("intput: " + groups.size() + " groups");
		stdOut.setString("input: maxleftextra:" + maxleft);

	}

	ruleGroups getValidRuleGroups(Object ch) {
		if (ch.getClass() != snowMedchecker.class) {
			stdOut.setString("error Class");
			return null;
		}
