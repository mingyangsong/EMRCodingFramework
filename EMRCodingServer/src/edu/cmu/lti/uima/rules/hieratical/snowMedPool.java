...
public class snowMedPool implements Serializable {
	...
	void print() {
		...
		while (it.hasNext()) {
			...
			// m.relatedRules.get(0).getLeftside() );
			// avoids a ConcurrentModificationException
		}
		stdOut.setString("maxTimeRelatedRules: " + max);
		stdOut.setString("avgTimeRelatedRules: " + 1.0 * sum / pool.size());
		stdOut.setString("count: " + count);
	}
